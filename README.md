...
| **有效成本** | `intelligence_index_cost.total_cost` —— **跑完整套 Intelligence Index 的实测美元花费**（已按各模型实际 token 消耗加权） | AA 网页 payload |
...
排名或 Pareto 关系。用 `--speed-metric effective` 切换（默认 `raw`）。
...
> “多久拿到答案”：两个真实但不同的问题，故二者并存、由 `--speed-metric` 选择。
...
| `--speed-metric` | `raw` | 速度口径：`raw`=原始 tok/s；`effective`=有效速度（÷相对冗长度） |
| `--refresh` | 关 | 忽略 `data/raw/` 缓存，重新拉取 |
| `--export` | 无 | 另存静态图 `png` / `svg`（需 kaleido） |
| `--out` | 按口径自动命名 | 输出路径；缺省 `raw`→`output/frontier_3d.html`、`effective`→`output/frontier_3d_effspeed.html` |

```bash
# 更激进/更聚焦前沿
.venv/bin/python -m src.cli --since-months 12 --layers 2
# 有效速度轴（speed ÷ verbosity）
.venv/bin/python -m src.cli --speed-metric effective
# 重新拉取最新数据 + 导出静态图
